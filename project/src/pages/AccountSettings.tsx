...
const AccountSettings = ({ user, onBack, onUpdateUser, onLogout }: AccountSettingsProps) => {
  ...
  const handleLogoutClick = async () => {
    ...
    try {
      ...
      showError(error instanceof Error ? error.message : 'No se pudo cerrar sesion.');
  ...
  return (
    <div className="min-h-screen bg-[#FFF8EF] text-[#2F3A35]">
      <div className="container mx-auto px-4 py-8">
        <button
          onClick={onBack}
          className="mb-6 inline-flex items-center gap-2 text-[#5FAE9B] transition-colors hover:text-[#4D9A88]"
        >
          <ArrowLeft className="h-5 w-5" />
          <span>Volver al inicio</span>
        </button>

        <div className="mb-8 rounded-xl border border-[#EADBC8]/70 bg-white/90 p-6 shadow-sm">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-[#6B756F]">{user.email}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="inline-flex items-center justify-center gap-2 self-end rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700 transition-colors hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60"
              >
                <LogOut className="h-4 w-4" />
                {isLoggingOut ? 'Cerrando...' : 'Cerrar sesión'}
              </button>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="rounded-lg border border-[#EADBC8] bg-[#FFF8EF] px-4 py-3">
                <p className="text-xs font-semibold uppercase text-[#6B756F]">Pedidos</p>
                <p className="mt-1 text-2xl font-semibold text-[#5FAE9B]">{stats.totalOrders}</p>
              </div>
              <div className="rounded-lg border border-[#EADBC8] bg-[#FFF8EF] px-4 py-3">
                <p className="text-xs font-semibold uppercase text-[#6B756F]">En curso</p>
                <p className="mt-1 text-2xl font-semibold text-[#D96C9F]">{stats.pending}</p>
              </div>
              <div className="rounded-lg border border-[#EADBC8] bg-[#FFF8EF] px-4 py-3">
                <p className="text-xs font-semibold uppercase text-[#6B756F]">Total comprado</p>
        ...
        {activeTab === 'profile' && (
          <form onSubmit={handleProfileSubmit} className="rounded-xl border border-[#EADBC8]/70 bg-white/90 p-6 shadow-sm">
            <div className="mb-6">
              ...
              <label className="block">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-[#EADBC8] py-3 pl-10 pr-4 focus:border-[#7FAF9B] focus:ring-2 focus:ring-[#7FAF9B]"
                    required
                  />
                </div>
              </label>
            ...
            <div className="mt-6 flex flex-col gap-3 border-t border-gray-100 pt-6 text-sm text-gray-600 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#5FAE9B] px-5 py-3 font-semibold text-white hover:bg-[#4D9A88] disabled:cursor-not-allowed disabled:opacity-60"
              >
        ...
        {activeTab === 'security' && (
          <section className="rounded-xl border border-[#EADBC8]/70 bg-white/90 p-6 shadow-sm">
            <div className="mb-6">
            ...
            <div className="rounded-lg border border-[#CFE3D4] bg-[#CFE3D4]/45 px-4 py-4 text-sm text-[#2F3A35]">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
                <Mail className="h-5 w-5 flex-shrink-0 text-[#5FAE9B]" />
                <div>
        ...
        {activeTab === 'orders' && (
          <div className="rounded-xl border border-[#EADBC8]/70 bg-white/90 p-6 shadow-sm">
                    ...
                    <div>
                      ...
                      {selectedOrder.deliveryMethod === 'delivery' && selectedOrder.customerInfo.address && (
                        <div className="mt-5 rounded-lg bg-gray-50 p-4 text-sm text-gray-700">
                          <p className="font-semibold text-gray-900">Direccion de entrega</p>
                      ...
                      <div className="mt-5 space-y-2 border-t border-gray-100 pt-4 text-sm">
                        ...
                        <div className="flex justify-between">
                          <span>{selectedOrder.deliveryMethod === 'pickup' ? 'Retiro' : 'Envio'}</span>
                          <span>
                            {selectedOrder.deliveryMethod === 'pickup'
                              ? 'Gratis'
