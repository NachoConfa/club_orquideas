...
begin
  ...
  loop
    ...
    if v_variant_id is not null then
      ...
      select coalesce(products.stock, 0), coalesce(products.price, 0), products.image_url
      into v_available_stock, v_product_price, v_product_image
    ...
    v_product_image := case
      when v_variant_id is not null then coalesce(nullif(v_variant_image_url, ''), nullif(v_product_image, ''), nullif(v_item->>'product_image', ''), nullif(v_item->>'image_url', ''))
      else coalesce(nullif(v_product_image, ''), nullif(v_item->>'product_image', ''), nullif(v_item->>'image_url', ''))
    end;
...
begin
  ...
  for v_item in
    ...
    group by order_items.product_id, order_items.variant_id
  loop
    ...
    if v_item.variant_id is not null then
      select coalesce(product_variants.stock, 0)
      into v_available_stock
      from public.product_variants
      join public.products on products.id = product_variants.product_id
      where product_variants.id = v_item.variant_id
  ...
  loop
    if v_item.variant_id is not null then
      ...
      where product_variants.id = v_item.variant_id
        and product_variants.product_id = v_item.product_id
        and coalesce(product_variants.stock, 0) >= v_item.quantity;
    else
      update public.products
      set stock = coalesce(stock, 0) - v_item.quantity
      where products.id = v_item.product_id
        and coalesce(products.stock, 0) >= v_item.quantity;
    end if;
...
drop policy if exists "Active product variants are public" on public.product_variants;
